...
from service.heuristics_service import HeuristicsService
from service.metrics_service import MetricsService
from config.settings import API_ENDPOINT
from components.ui_components import dynamic_metric
...
def home_tab(T):
    ...
    with st.sidebar.expander(T["search_expander"]):
        ...
        if st.button(T["search"]):
            applications = metrics_service.list_applications(status=status, min_date=date_to_str(min_date), max_date=date_to_str(max_date), limit=limit)
        else:
            applications = metrics_service.list_applications(limit=limit)

        app_options = [f"{app['id']} - {app['name']}" for app in applications]
        ...
        if selected_app:
            ...
            attempt_options = [f"{i+1}: {a['startTime']} ({T['attempt_duration']}: {a['duration']} ms)" for i, a in enumerate(attempts)]
    ...
    if st.sidebar.button(T["generate"]):
        if application_id:
            ...
            with row2a:
                dynamic_metric(row2a, "💾 Mean Heap Usage", round(metrics_service.get_ratio_on_heap_memory()*100, 2), 50, 80)
            with row2b:
                dynamic_metric(row2b, "💾 Max Heap Usage", round(metrics_service.get_max_ratio_on_heap_memory()*100, 2), 50, 80)
            with row2c:
                dynamic_metric(row2c, "⚙️ CPU Usage", 75, 50, 80)
            with row2d:
                dynamic_metric(row2d, "📊 Disk Space", 40, 50, 80)
            with row2e:
                dynamic_metric(row2e, "📊 Failed Task", 94, 0, 1, unit="")
